...
static bool esc_cali_flag = 1; //电调校准标志，上电只校准一次
...
void RC_duty_task(u8 dT_ms) //建议2ms调用一次
{
  if(flag.start_ok) {
    ...
    if(esc_cali_flag == 1 && CH_N[AUX4] > 490 && CH_N[CH_THR] > 400 && CH_N[CH_YAW] < -400) {
      ANO_DT_SendString("Start Calibrate ESC!!!! Please Waite");
      ...
      esc_cali_flag = 0;  //关闭校准功能
      ANO_DT_SendString("ESC Calibrate Over!!!");
