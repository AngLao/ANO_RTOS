...
//---	姿态控制角速度环PID参数
  ...
  Ano_Parame.set.pid_att_1level[ROL][KI] = 2.5f; //姿态控制角速度环PID参数
  Ano_Parame.set.pid_att_1level[ROL][KD] = 0.10f; //姿态控制角速度环PID参数

  Ano_Parame.set.pid_att_1level[PIT][KP] = 4.1f; //姿态控制角速度环PID参数
  Ano_Parame.set.pid_att_1level[PIT][KI] = 2.5f; //姿态控制角速度环PID参数
  Ano_Parame.set.pid_att_1level[PIT][KD] = 0.10f; //姿态控制角速度环PID参数

  Ano_Parame.set.pid_att_1level[YAW][KP] = 4.5f; //姿态控制角速度环PID参数
  Ano_Parame.set.pid_att_1level[YAW][KI] = 0.3f; //姿态控制角速度环PID参数
  Ano_Parame.set.pid_att_1level[YAW][KD] = 0.0f; //姿态控制角速度环PID参数
//---	姿态控制角度环PID参数
  Ano_Parame.set.pid_att_2level[ROL][KP] = 7.8f; //姿态控制角度环PID参数
  Ano_Parame.set.pid_att_2level[ROL][KI] = 0.00f; //姿态控制角度环PID参数
  Ano_Parame.set.pid_att_2level[ROL][KD] = 0.00f; //姿态控制角度环PID参数

  Ano_Parame.set.pid_att_2level[PIT][KP] = 7.6f; //姿态控制角度环PID参数
  Ano_Parame.set.pid_att_2level[PIT][KI] = 0.00f; //姿态控制角度环PID参数
  Ano_Parame.set.pid_att_2level[PIT][KD] = 0.00f; //姿态控制角度环PID参数

  Ano_Parame.set.pid_att_2level[YAW][KP] = 5.5f; //姿态控制角度环PID参数
  Ano_Parame.set.pid_att_2level[YAW][KI] = 0.0f; //姿态控制角度环PID参数
  Ano_Parame.set.pid_att_2level[YAW][KD] = 0.5; //姿态控制角度环PID参数
//---	高度控制高度速度环PID参数 
  Ano_Parame.set.pid_alt_1level[KP] = 2.0f;          //高度控制高度速度环PID参数
...
{
  for(u8 i = 0; i < 3; i++) {
    save.acc_offset[i]		=	Ano_Parame.set.acc_zero_offset[i];
    save.gyro_offset[i]		=	Ano_Parame.set.gyr_zero_offset[i];
...
static void Parame_Copy_Fc2para()
{
  ...
  for(u8 i = 0; i < 3; i++) {
    Ano_Parame.set.acc_zero_offset[i]	=	save.acc_offset[i];
...
s32 AnoParRead(u16 _id)
{
  ...
  case PAR_LV_WARN_100:
    p_val = 1e2f * Ano_Parame.set.warn_power_voltage;
    break;

  case PAR_LV_LAND_100:
    p_val = 1e2f * Ano_Parame.set.lowest_power_voltage;
    break;

  case PAR_HEATSWITCH:
    p_val = Ano_Parame.set.heatSwitch;
    break;

  case PAR_LANDSPEED:
    p_val = Ano_Parame.set.auto_landing_speed;
    break;

  case PAR_TAKEOFFSPEED:
    p_val = Ano_Parame.set.auto_take_off_speed;
    break;
...
//    break;
  ...
  //角速度
  case PAR_PID_1_P:
    p_val = 1e3f * Ano_Parame.set.pid_att_1level[ROL][0];
    break;

  case PAR_PID_1_I:
    p_val = 1e3f * Ano_Parame.set.pid_att_1level[ROL][1];
    break;

  case PAR_PID_1_D:
    p_val = 1e3f * Ano_Parame.set.pid_att_1level[ROL][2];
    break;

  case PAR_PID_2_P:
    p_val = 1e3f * Ano_Parame.set.pid_att_1level[PIT][0];
    break;

  case PAR_PID_2_I:
    p_val = 1e3f * Ano_Parame.set.pid_att_1level[PIT][1];
    break;

  case PAR_PID_2_D:
    p_val = 1e3f * Ano_Parame.set.pid_att_1level[PIT][2];
    break;

  case PAR_PID_3_P:
    p_val = 1e3f * Ano_Parame.set.pid_att_1level[YAW][0];
    break;

  case PAR_PID_3_I:
    p_val = 1e3f * Ano_Parame.set.pid_att_1level[YAW][1];
    break;

  case PAR_PID_3_D:
    p_val = 1e3f * Ano_Parame.set.pid_att_1level[YAW][2];
    break;

  //角度
  case PAR_PID_4_P:
    p_val = 1e3f * Ano_Parame.set.pid_att_2level[ROL][0];
    break;

  case PAR_PID_4_I:
    p_val = 1e3f * Ano_Parame.set.pid_att_2level[ROL][1];
    break;

  case PAR_PID_4_D:
    p_val = 1e3f * Ano_Parame.set.pid_att_2level[ROL][2];
    break;

  case PAR_PID_5_P:
    p_val = 1e3f * Ano_Parame.set.pid_att_2level[PIT][0];
    break;

  case PAR_PID_5_I:
    p_val = 1e3f * Ano_Parame.set.pid_att_2level[PIT][1];
    break;

  case PAR_PID_5_D:
    p_val = 1e3f * Ano_Parame.set.pid_att_2level[PIT][2];
    break;

  case PAR_PID_6_P:
    p_val = 1e3f * Ano_Parame.set.pid_att_2level[YAW][0];
    break;

  case PAR_PID_6_I:
    p_val = 1e3f * Ano_Parame.set.pid_att_2level[YAW][1];
    break;

  case PAR_PID_6_D:
    p_val = 1e3f * Ano_Parame.set.pid_att_2level[YAW][2];
    break;

  //垂直速度
  case PAR_PID_7_P:
    p_val = 1e3f * Ano_Parame.set.pid_alt_1level[0];
    break;

  case PAR_PID_7_I:
    p_val = 1e3f * Ano_Parame.set.pid_alt_1level[1];
    break;

  case PAR_PID_7_D:
    p_val = 1e3f * Ano_Parame.set.pid_alt_1level[2];
    break;
...
//    break;
  ...
  case CAL_ACC_OFFSET_Y:
    p_val = Ano_Parame.set.acc_zero_offset[1] * 1000;
    break;

  case CAL_ACC_OFFSET_Z:
    p_val = Ano_Parame.set.acc_zero_offset[2] * 1000;
    break;

  case CAL_ACC_SENSIV_X:
    p_val = Ano_Parame.set.acc_sensitivity_ref[0] * 1000;
    break;

  case CAL_ACC_SENSIV_Y:
    p_val = Ano_Parame.set.acc_sensitivity_ref[1] * 1000;
    break;

  case CAL_ACC_SENSIV_Z:
    p_val = Ano_Parame.set.acc_sensitivity_ref[2] * 1000;
    break;

  case CAL_ACC_IEM_00:
    p_val = Ano_Parame.set.IEM[0][0] * 100000;
    break;

  case CAL_ACC_IEM_01:
    p_val = Ano_Parame.set.IEM[0][1] * 100000;
    break;

  case CAL_ACC_IEM_02:
    p_val = Ano_Parame.set.IEM[0][2] * 100000;
    break;

  case CAL_ACC_IEM_10:
    p_val = Ano_Parame.set.IEM[1][0] * 100000;
    break;

  case CAL_ACC_IEM_11:
    p_val = Ano_Parame.set.IEM[1][1] * 100000;
    break;

  case CAL_ACC_IEM_12:
    p_val = Ano_Parame.set.IEM[1][2] * 100000;
    break;

  case CAL_ACC_IEM_20:
    p_val = Ano_Parame.set.IEM[2][0] * 100000;
    break;

  case CAL_ACC_IEM_21:
    p_val = Ano_Parame.set.IEM[2][1] * 100000;
    break;

  case CAL_ACC_IEM_22:
    p_val = Ano_Parame.set.IEM[2][2] * 100000;
    break;

  case CAL_MAG_OFFSET_X:
    p_val = Ano_Parame.set.mag_offset[0] * 1000;
    break;

  case CAL_MAG_OFFSET_Y:
    p_val = Ano_Parame.set.mag_offset[1] * 1000;
    break;

  case CAL_MAG_OFFSET_Z:
    p_val = Ano_Parame.set.mag_offset[2] * 1000;
    break;

  case CAL_MAG_SENSIV_X:
    p_val = Ano_Parame.set.mag_gain[0] * 1000;
    break;

  case CAL_MAG_SENSIV_Y:
    p_val = Ano_Parame.set.mag_gain[1] * 1000;
    break;

  case CAL_MAG_SENSIV_Z:
    p_val = Ano_Parame.set.mag_gain[2] * 1000;
    break;
...
void AnoParWrite(u16 _id, s32 _val)
{
  ...
  //====设置
  case PAR_BAT_CELLS:
    Ano_Parame.set.bat_cell = LIMIT(_val, 0, 65535);
    break;

  case PAR_LV_WARN_100:
    Ano_Parame.set.warn_power_voltage = 1e-2f * LIMIT(_val, 0, 65535);
    break;

  case PAR_LV_LAND_100:
    Ano_Parame.set.lowest_power_voltage = 1e-2f * LIMIT(_val, 0, 65535);
    break;

  case PAR_HEATSWITCH:
    Ano_Parame.set.heatSwitch = LIMIT(_val, 0, 2);
    break;

  case PAR_LANDSPEED:
    Ano_Parame.set.auto_landing_speed = LIMIT(_val, 0, 65535);
    break;

  case PAR_TAKEOFFSPEED:
    Ano_Parame.set.auto_take_off_speed = LIMIT(_val, 0, 65535);
    break;
  ...
  case PAR_UNLOCKPWM:
    Ano_Parame.set.idle_speed_pwm = LIMIT(_val, 0, 65535);
    break;

  //====PID
  //角速度
  case PAR_PID_1_P:
    Ano_Parame.set.pid_att_1level[ROL][0] = 1e-3f  * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_1_I:
    Ano_Parame.set.pid_att_1level[ROL][1] = 1e-3f  * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_1_D:
    Ano_Parame.set.pid_att_1level[ROL][2] = 1e-3f  * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_2_P:
    Ano_Parame.set.pid_att_1level[PIT][0] = 1e-3f  * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_2_I:
    Ano_Parame.set.pid_att_1level[PIT][1] = 1e-3f  * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_2_D:
    Ano_Parame.set.pid_att_1level[PIT][2] = 1e-3f  * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_3_P:
    Ano_Parame.set.pid_att_1level[YAW][0] = 1e-3f  * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_3_I:
    Ano_Parame.set.pid_att_1level[YAW][1] = 1e-3f  * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_3_D:
    Ano_Parame.set.pid_att_1level[YAW][2] = 1e-3f  * LIMIT(_val, 0, 65535);
    break;

  //角度
  case PAR_PID_4_P:
    Ano_Parame.set.pid_att_2level[ROL][0] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_4_I:
    Ano_Parame.set.pid_att_2level[ROL][1] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_4_D:
    Ano_Parame.set.pid_att_2level[ROL][2] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_5_P:
    Ano_Parame.set.pid_att_2level[PIT][0] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_5_I:
    Ano_Parame.set.pid_att_2level[PIT][1] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_5_D:
    Ano_Parame.set.pid_att_2level[PIT][2] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_6_P:
    Ano_Parame.set.pid_att_2level[YAW][0] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_6_I:
    Ano_Parame.set.pid_att_2level[YAW][1] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_6_D:
    Ano_Parame.set.pid_att_2level[YAW][2] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  //垂直速度
  case PAR_PID_7_P:
    Ano_Parame.set.pid_alt_1level[0] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_7_I:
    Ano_Parame.set.pid_alt_1level[1] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_7_D:
    Ano_Parame.set.pid_alt_1level[2] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  //高度
  case PAR_PID_8_P:
    Ano_Parame.set.pid_alt_2level[0] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_8_I:
    Ano_Parame.set.pid_alt_2level[1] = 0;
    break;//LIMIT(p_val,0,65535);	break;

  case PAR_PID_8_D:
    Ano_Parame.set.pid_alt_2level[2] = 0;
    break;//LIMIT(p_val,0,65535);	break;

  //水平速度
  case PAR_PID_9_P:
    Ano_Parame.set.pid_loc_1level[0] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_9_I:
    Ano_Parame.set.pid_loc_1level[1] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_9_D:
    Ano_Parame.set.pid_loc_1level[2] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  //水平位置
  case PAR_PID_10_P:
    Ano_Parame.set.pid_loc_2level[0] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_10_I:
    Ano_Parame.set.pid_loc_2level[1] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_10_D:
    Ano_Parame.set.pid_loc_2level[2] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  //
  case PAR_PID_11_P:
    Ano_Parame.set.pid_gps_loc_1level[0] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_11_I:
    Ano_Parame.set.pid_gps_loc_1level[1] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_11_D:
    Ano_Parame.set.pid_gps_loc_1level[2] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  //
  case PAR_PID_12_P:
    Ano_Parame.set.pid_gps_loc_2level[0] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_12_I:
    Ano_Parame.set.pid_gps_loc_2level[1] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  case PAR_PID_12_D:
    Ano_Parame.set.pid_gps_loc_2level[2] = 1e-3f * LIMIT(_val, 0, 65535);
    break;

  //====sensor_cali
  case CAL_ACC_OFFSET_X:
    Ano_Parame.set.acc_zero_offset[0] = (float)_val / 1000;
    break;

  case CAL_ACC_OFFSET_Y:
    Ano_Parame.set.acc_zero_offset[1] = (float)_val / 1000;
    break;

  case CAL_ACC_OFFSET_Z:
    Ano_Parame.set.acc_zero_offset[2] = (float)_val / 1000;
    break;

  case CAL_ACC_SENSIV_X:
    Ano_Parame.set.acc_sensitivity_ref[0] = (float)_val / 1000;
    break;

  case CAL_ACC_SENSIV_Y:
    Ano_Parame.set.acc_sensitivity_ref[1] = (float)_val / 1000;
    break;

  case CAL_ACC_SENSIV_Z:
    Ano_Parame.set.acc_sensitivity_ref[2] = (float)_val / 1000;
    break;

  case CAL_ACC_IEM_00:
    Ano_Parame.set.IEM[0][0] = (float)_val / 100000;
    break;

  case CAL_ACC_IEM_01:
    Ano_Parame.set.IEM[0][1] = (float)_val / 100000;
    break;

  case CAL_ACC_IEM_02:
    Ano_Parame.set.IEM[0][2] = (float)_val / 100000;
    break;

  case CAL_ACC_IEM_10:
    Ano_Parame.set.IEM[1][0] = (float)_val / 100000;
    break;

  case CAL_ACC_IEM_11:
    Ano_Parame.set.IEM[1][1] = (float)_val / 100000;
    break;

  case CAL_ACC_IEM_12:
    Ano_Parame.set.IEM[1][2] = (float)_val / 100000;
    break;

  case CAL_ACC_IEM_20:
    Ano_Parame.set.IEM[2][0] = (float)_val / 100000;
    break;

  case CAL_ACC_IEM_21:
    Ano_Parame.set.IEM[2][1] = (float)_val / 100000;
    break;
...
//			data_save();
  ...
  case CAL_MAG_OFFSET_X:
    Ano_Parame.set.mag_offset[0] = (float)_val / 1000;
    break;

  case CAL_MAG_OFFSET_Y:
    Ano_Parame.set.mag_offset[1] = (float)_val / 1000;
    break;

  case CAL_MAG_OFFSET_Z:
    Ano_Parame.set.mag_offset[2] = (float)_val / 1000;
    break;

  case CAL_MAG_SENSIV_X:
    Ano_Parame.set.mag_gain[0] = (float)_val / 1000;
    break;

  case CAL_MAG_SENSIV_Y:
    Ano_Parame.set.mag_gain[1] = (float)_val / 1000;
    break;
