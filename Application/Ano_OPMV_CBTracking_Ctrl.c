...
static void ANO_CBTracking_Decoupling(u8 *dT_ms,float rol_degs,float pit_degs)
{
	...
	//换到飞控坐标系
	ano_opmv_cbt_ctrl.opmv_pos[0] =  opmv.cb.pos_y * CMP_KP;
	ano_opmv_cbt_ctrl.opmv_pos[1] =  opmv.cb.pos_x * CMP_KP;
//	ano_opmv_cbt_ctrl.opmv_pos[0] =  -40 * CMP_KP;
//	ano_opmv_cbt_ctrl.opmv_pos[1] =  -( -40 ) * CMP_KP;
	//
	if(opmv.cb.sta != 0)
	{
		//更新姿态量对应的偏移量
		...
		ano_opmv_cbt_ctrl.rp2pixel_val[0] = LIMIT(ano_opmv_cbt_ctrl.rp2pixel_val[0],-120,120);//高度240pixel
		ano_opmv_cbt_ctrl.rp2pixel_val[1] = LIMIT(ano_opmv_cbt_ctrl.rp2pixel_val[1],-160,160);//宽度320pixel
		//赋值参考的载体运动速度
		if(OF_DATA_SOURCE==1)
		{
			//来源1  ANO_OF
